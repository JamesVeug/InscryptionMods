...
public static class RandomOptions
{
	...
	static void OnChangeSigils(CardInfo cardByName, CardModificationInfo copyMod)
	{
		...
		modsWithAbilities.Sort((a, b) => SeededRandom.Bool(CopyCard_Patches.Seed++) ? 1 : -1);
		vanillaAbilitiesIndex.Sort((a, b) => SeededRandom.Bool(CopyCard_Patches.Seed++) ? 1 : -1);
		...
		int shuffles = !Configs.CanShuffleAllSigils.Value
			? 1
			: SeededRandom.Range(1, modsWithAbilities.Count + vanillaAbilities.Count, CopyCard_Patches.Seed++);
		...
		for (int i = 0; i < shuffles; i++)
		{
			...
			{
				chooseMod = SeededRandom.Range(0, modWeight + vanillaWeight, CopyCard_Patches.Seed++) < modWeight;
			}
			else if (modWeight > 0)
			{
				chooseMod = true;
			...
			if (Configs.SuperMode.Value && shuffles > 1)
			{
				...
				index2 = SeededRandom.Range(minRange, maxRange, CopyCard_Patches.Seed++);
			}
			else
			{
				index2 = SeededRandom.Range(0, learnedAbilities.Count, CopyCard_Patches.Seed++);
			}
			...
			if (chooseMod)
			{
				...
				int index = SeededRandom.Range(0, modsWithAbilities[0].abilities.Count, CopyCard_Patches.Seed++);
	...
	static void OnAddSigils(CardInfo cardByName, CardModificationInfo copyMod)
	{
		...
		int amount = Configs.SuperMode.Value ? SeededRandom.Range(1, 2, CopyCard_Patches.Seed++) : 1;
		...
		{
			Ability seededRandom = list.GetSeededRandom(CopyCard_Patches.Seed++);
	...
	static void OnRemoveSigil(CardInfo cardByName, CardModificationInfo copyMod)
	{
		...
		for (int i = 0; i < amount; i++)
		{
			...
			CardModificationInfo info = list.GetSeededRandom(CopyCard_Patches.Seed++);
			info.abilities.Remove(info.abilities.GetSeededRandom(CopyCard_Patches.Seed++));
		}
	}
	...
	static void OnChangeAttack(CardInfo cardByName, CardModificationInfo copyMod)
	{
		...
		bool increase = SeededRandom.Bool(CopyCard_Patches.Seed++);
	...
	{
		bool increase = cardByName.tribes.Count <= 0 || SeededRandom.Bool(CopyCard_Patches.Seed++);
		...
		Tribe newTribe = allTribes.GetSeededRandom(CopyCard_Patches.Seed++);
		if (increase)
		{
		...
		{
			int i = SeededRandom.Range(0, cardByName.tribes.Count, CopyCard_Patches.Seed++);
	...
	public static void OnNameChange(CardInfo cardByName, CardModificationInfo copyMod)
	{
		...
		int amountToChange = 1;//SeededRandom.Range(1, 2, CopyCard_Patches.Seed++);
		for (int i = 0; i < amountToChange && cardByName.displayedName.Length > 2; i++)
		{
			int indexToChange = SeededRandom.Range(1, cardByName.displayedName.Length - 1, CopyCard_Patches.Seed++);
			...
			if (c == cardByName.displayedName[indexToChange - 1] && SeededRandom.Bool(CopyCard_Patches.Seed++))
			{
				// SS => S
			...
			else if (replacements.TryGetValue(c, out string[] others) && others.Length > 0)
			{
				...
				int replacementIndex = SeededRandom.Range(0, others.Length, CopyCard_Patches.Seed++);
	...
	static void OnChangeCost(CardInfo cardByName, CardModificationInfo copyMod)
	{
		...
		else if (cardByName.GemsCost != null && cardByName.GemsCost.Count > 0)
		{
			if (increase)
			{
				...
				{
					int index = SeededRandom.Range(0, gemTypes.Count, CopyCard_Patches.Seed++);
			...
			else
			{
				...
				{
					int index = SeededRandom.Range(0, allGems.Count, CopyCard_Patches.Seed++);
